...
- During implementation, run only focused tests that cover the files or behavior
  being changed.
- Do not run tests immediately after every code change.
- First implement the change and show the result to the user for verification.
- Run the full test suite only after the user has verified the result, or when
  the current change includes creating or updating the relevant test file(s).
